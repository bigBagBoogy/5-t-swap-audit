...
A `harness file` is a file you can code as a work-around for functions that are private
in the original contract. Also here you can create `getters` for variables you need.
So for example: ***TSwapPoolHarness.sol***
The harness contract should inherit fron the original contract.

## .conf  JSON5 layout:
***this is taken off the CVL1 code***

1.  ```certoraRun certora/harness/TSwapPoolHarness.sol:TSwapPool```

above line broken down: from root, into /certora, into /harness, run file: 
TSwapPoolHarness.sol, and of that, contract `TSwapPool` (for when multiple contracts
 in 1 file).

2. ```--verify TSwapPool:certora/specs/setup.spec```
means: verify contract and use: <path-to-your-spec-file>

3.  the rest (optimistic loop and flags)
### optimistic loop flag?
This avoids warnings for strings and loops. Since the unwinding of a loop is
a `difficult` thing, by default, the prover assumes a fail. To avoid this 
behavior, we use the `--optimistic loop` flag

### tip on `view` functions:
since we're always checking on `state` changes, functions that are `view`-functions
