...
// methods {
//     function _isUnknown(IERC20 token) external returns bool;    
// }
...
rule testInvalidToken(method f) {
    ...
    assert (!isUnknown <=> inputToken == i_wethToken || inputToken == i_poolToken, "foreign token comes into the protocol");
}
